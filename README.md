...
[pyramid_oereb.yaml](https://github.com/camptocamp/pyramid_oereb/blob/master/pyramid_oereb/standard/pyramid_oereb.yml).
...
applications code to not mess things up.
...
handle spacial data (postgis,...). **This connections are used later to create/drop standard tables and
...
```
13. Check with a tool of your choise that the structure was created successfully in you desired database.
You should find 17 database schemas named (snake_case) by their code attribute from the yml file. Plus one
schema called "pyramid_oereb_main" containing the app global stuff (addresses, municipalities, etc.). At
least these tables need to be filled up with your data with a tool of your choise).
...
13. Now to the configuration - you could do a commit and push on Git to have a clean project before
...
and adapt the necessary parameters to your environment - e.g. `db_connection` and so on
